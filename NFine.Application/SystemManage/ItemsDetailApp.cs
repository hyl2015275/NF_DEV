...
namespace NFine.Application.SystemManage
{
    public class ItemsDetailApp
    {
        ...
        public string GetKeyByValue(string majorClass, string value)
        {
            var dataItems = (Dictionary<string, object>)GetDataItemList();
            return ((Dictionary<string, string>)dataItems[majorClass]).Where(q => q.Value != null && q.Value == value).Select(q => q.Key).FirstOrDefault();
        }
    }
}
